...
export default class Shop {
    game: Game;
    menu: HTMLElement;
    constructor(game: Game, menu: HTMLElement) {
        this.game = game;
        this.menu = menu;
        this.createShop();
    }
    ...
    buy(name: string, cost: number, type: string, callback: Function): boolean {
        let purchase = false
        switch(type) {
            case "Social Credit":
    ...
    grassLvlUpCost() {
        ...
        if(level >= 100) levelupCost = level * 3500
        if(level >= 90) levelupCost = level * 2750
        if(level >= 80) levelupCost = level * 2050
        if(level >= 70) levelupCost = level * 1750
        if(level >= 60) levelupCost = level * 1550
        if(level >= 50) levelupCost = level * 1250
        if(level >= 40) levelupCost = level * 1050
        if(level >= 30) levelupCost = level * 850
        if(level >= 20) levelupCost = level * 650
        if(level >= 10) levelupCost = level * 450
        if(level < 10) levelupCost = level * 150
        return levelupCost;
    }
